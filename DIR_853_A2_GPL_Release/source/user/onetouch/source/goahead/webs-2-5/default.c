...
/* DLINK add/s [Jack: 2014/11/06] */
char newUserSessionID[COOKIE_MAXLEN];

void init_DHttpd_WebsRec(webs_t wp, DHttpd_WebsRecPtr dwp)
{
	static int isByPassLogin = -1;

	if (isByPassLogin==-1) 
		isByPassLogin = (xget_int("/sys/defaultbypasslogin")!=0)? 1: 0;
		
	// Setting Config
	dwp->webDefaultDir = websGetDefaultDir();
	dwp->url = wp->url;
	dwp->cookie = wp->cookie;	
	dwp->ipaddr = wp->ipaddr;
	dwp->postData = wp->postData;
	dwp->lenPostData = wp->lenPostData;
	dwp->isChkPath = 0;						// check path was done before call dhttpd_WebsHandler
	dwp->isByPassLogin = isByPassLogin;		// by pass login page when default setting
	// Init result data
	dwp->ret_data = NULL;
	dwp->isNoCache = 0;
	dwp->isNewUser = 0;
	dwp->currAuthUser = NULL;
}
/* DLINK add/e */

#define RssUI_DebugLogURL		"/tmp/" _OT_LogHeader "_log.txt"

int websDefaultHandler(webs_t wp, char_t *urlPrefix, char_t *webDir, int arg,
						char_t *url, char_t *path, char_t *query)
{
	LOG_DEBUG_TIME("url:%s,query:%s,cookie:'%s'\n",wp->url,wp->query,wp->cookie);
/* DLINK add/s [Jack: 2014/11/05] */
	int isNoCache = FALSE, isRedirectLogin = TRUE;

	if ( MATCH_PREFIX(wp->url, "/rssui/") || MATCH_PREFIX(wp->url, "/rsstest/") || 
		 MATCH_PREFIX(wp->url, "/tmp/rss") || MATCH_PREFIX(wp->url, RssUI_DebugLogURL) ) {
		DHttpd_WebsRec	dWebsRec;

		init_DHttpd_WebsRec(wp, &dWebsRec);
		int ii = dhttpd_WebsHandler(&dWebsRec);
		isRedirectLogin = FALSE;
		switch (ii) 
		{
			case -1:  	// bypass, nothing to do, if not exist, redirect to login pages
			case 0:  	// nothing to do or response file was prepared, if not exist, response 404, unknow pages
				break;
			case 1: 	// redirect to URL:"dWebsRec.ret_data"
				if (dWebsRec.isNewUser && dWebsRec.currAuthUser)
					strcpy(newUserSessionID, dWebsRec.currAuthUser->sessionID); 
				websRedirect(wp, T(dWebsRec.ret_data));
				LOG_DEBUG("redirect to '%s'\n", dWebsRec.ret_data);
				break;
			case 2: 	// use "dWebsRec.ret_data" to generate return HTML packet
				websHeader2(wp);
			case 3: 	// use "dWebsRec.ret_data" to generate return XML packet
				if (ii == 3)  websXmlHeader(wp);
				if (dWebsRec.ret_data) websWrite(wp, T(dWebsRec.ret_data));
				websDone(wp, 200);
				break;
		}
		isNoCache = dWebsRec.isNoCache;
		free_DHttpdWebsRec(&dWebsRec);
		if ((ii != 0)&&(ii != -1))  return 1;
	}
/* DLINK add/e [Jack: 2014/11/05] */
 ...
 */
 	LOG_DEBUG("lpath=%s, path=%s\n", lpath, path);//carlos debug
		...
		0666) < 0) 
   {
       ...
       */
        /* DLINK add/s [Jack: 2015/09/22, redirect ro login page when unknow url] */
        if (isRedirectLogin) { 
			websRedirect(wp, T(generateLoginUrl()));
			LOG_DEBUG("Cannot open URL '%s', redirect to login page.\n", wp->url);
			return 1;
        }
       /* DLINK add/e [Jack: 2015/09/22] */
		websError(wp, 404, T("Cannot open URL"));
		websBuildWhitelist();
		return 1;
	} 
 ...
 */
	//if (flags & WEBS_ASP) {
	if ((flags & WEBS_ASP) || isNoCache) {  /* DLINK mod [Jack: 2014/11/05] */ 
		bytes = 0;
	...
	if ((flags & WEBS_KEEP_ALIVE) && !((flags & WEBS_ASP) || isNoCache)) {  /* DLINK mod [Jack: 2014/11/05] */
		websWrite(wp, T("Connection: keep-alive\r\n"));
	}
	websWrite(wp, T("\r\n"));
