...
static int		upgrade_fd = -1; /* DLINK add [Vincent: 2015/03/25: For FW upgrading]*/
...
/* DLINK mod/s [Vincent: 2015/03/25: For FW upgrading]*/
    char_t  *text, *ptr, *ptr2;
    int     rc, nbytes, len, done, fd, size;
    char tmp[64] = {0};
    char fwfn[128] = {0};
	int upgrade_len = 0, count = 0;
/* DLINK mod/e */
 ...
 */
		while ((rc = websGetInput(wp, &text, &nbytes)) == 0) {
			usleep(100); /* DLINK mod [Vincent: 2015/03/25: For FW upgrading]*/
		}
 ...
 */
		...
		/* DLINK mod/s [Vincent: 2015/03/25: For FW upgrading]*/
            if (wp->flags & WEBS_CGI_FIRMWARE_UPLOAD) {
				upgrade_len = nbytes;
                if (upgrade_fd == -1) {
					do_prepare_fwupload(wp);// DLINK mod [Jack: 2015/04/28: For FW upgrading, to prepare the ram disk space to store upload FW]
                    ...
                    strcat(fwfn, tmp);
                    LOG_DEBUG("\nsavedfn='%s' ....................................\n\n", fwfn);
                    upgrade_fd = gopen(fwfn, O_CREAT | O_WRONLY | O_BINARY,
                        0666);
					//fix header
                    if (ptr = strstr(text,"Content-Type:")) {
						LOG_DEBUG("text='%s' upgrade_len=%d nbytes=%d\n\n", text, upgrade_len, nbytes);
                        if (ptr2 = strstr(ptr, "\r\n\r\n")) {
							*ptr2 = '\0';
							upgrade_len = nbytes - strlen(text) - 4;
                            text = ptr2 + 4;
                        }
						LOG_DEBUG("text='%s' upgrade_len=%d nbytes=%d\n\n", text, upgrade_len, nbytes);
                    }
				...
				//fix tail
				if ( ( wp->clen - nbytes ) < nbytes && wp->multiPartBoundary) {
					LOG_DEBUG("wp->multiPartBoundary=%s, nbytes=%d\n\n", wp->multiPartBoundary, nbytes);
					count = 0;
					ptr = text;
					while(1) {
						if (count > nbytes)
						...
						if (*ptr) {
							if (ptr2 = strstr(ptr, wp->multiPartBoundary)) {
								*ptr2 = '\0';
								upgrade_len = count + strlen(ptr) - 2;
								LOG_DEBUG("ptr2='%s' upgrade_len=%d #### nbytes=%d\n\n", ptr2 + 1, upgrade_len, nbytes);
								break;
							}
						}
						ptr++;
                ...
                gwrite(upgrade_fd, text, upgrade_len);
            }
            else if (wp->postData) {
		/* DLINK mod/e */
#if 0
...
/* DLINK add/s [Vincent: 2015/03/25: For FW upgrading]*/
    if(gstrstr(url, CGI_FIRMWARE_UPLOAD) != NULL){
		/* DLINK add/s [Vincent: 2015/06/10]*/
		// fix multiple post FW issue in same time
		if (xget_int(PATH_RtFwUpgInfo "/state") >= 1)
			return -1;
		/* DLINK add/e */
        wp->flags |= WEBS_CGI_FIRMWARE_UPLOAD;
        LOG_DEBUG_TIME("FW Upgrade: url=%s\n", url);
    }
/* DLINK add/e */
...
/* DLINK add/s [Jack: 2014/10/31, For login sessionid auth] */
extern 	char newUserSessionID[];  // predefined in default.c 
/* DLINK add/e [Jack: 2014/10/31] */
 ...
 */
		if (redirect) {
			...
			/* DLINK add/s [Jack: 2014/10/31, For login sessionid auth] */
			if (newUserSessionID[0])	{
				websWrite(wp, T("Set-Cookie: %s%s;path=/\r\n"), SESSIONID_HEADER, newUserSessionID);
				newUserSessionID[0] = '\0';  // clear it, due to avoid to set cookie again in next packet 
			}
			/* DLINK add/e [Jack: 2014/10/31] */
		}
		websWrite(wp, T("\r\n"));
...
void websFree(webs_t wp)
{
	...
	/* DLINK add/s [Carlos: 2014/07/11: Fix goahead crash bug by mydlink portal ] */
	if(wp->wid)
	/* DLINK add/e */
