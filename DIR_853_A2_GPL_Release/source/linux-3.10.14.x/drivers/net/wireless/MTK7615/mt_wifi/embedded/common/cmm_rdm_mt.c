...
static inline BOOLEAN DfsCheckChAvailableByBw(
	...
	IN PRTMP_ADAPTER pAd, UCHAR ucRddIdx)
{
	...
	BackgroundScanCancelAction(pAd, NULL); // Cannel background scan.
	    ...
	    MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s]SrvMonitor State & RadarDetected=%d\n", __FUNCTION__,
						    BgndScanCtrl->RadarDetected));
	    MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\x1b[1;33m [%s]PrimCh: %d, Band0Ch:%d, Band1Ch:%d \x1b[m \n", 
	    ...
	    MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s] RD_NORMAL_MODE. Update Uniform Ch=%d\n", 
					    __FUNCTION__, NextCh));
		...
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s] RD_SILENCE_MODE. Update Uniform Ch=%d\n", 
				__FUNCTION__, NextCh));			
			MlmeEnqueue(pAd, DFS_STATE_MACHINE, DFS_CHAN_SWITCH_TIMEOUT, 0, NULL, 0);			
 		    RTMP_MLME_HANDLER(pAd);
        }
	...
	IN PRTMP_ADAPTER pAd, PDFS_PARAM pDfsParam, BOOLEAN bSkipDfsCh, UCHAR Bw8080ChAvoid)
{
	...
	if(pDfsParam->bIEEE80211H)		
	{
	    for (i = 0; i < pDfsParam->ChannelListNum; i++)
	    {
			...
			if(!DfsCheckChAvailableByBw(pDfsParam->DfsChannelList[i].Channel, pDfsParam->Bw, pDfsParam))
 		        continue;
	
			if((Bw8080ChAvoid != 0) 
			&& vht_cent_ch_freq(pDfsParam->DfsChannelList[i].Channel, VHT_BW_80) == vht_cent_ch_freq(Bw8080ChAvoid, VHT_BW_80))
 				continue;
		...
		{			
			...
			{
				if(DfsCheckChAvailableByBw(pDfsParam->DfsChannelList[i].Channel, pDfsParam->Bw, pDfsParam)
				&& (vht_cent_ch_freq(pDfsParam->DfsChannelList[i].Channel, VHT_BW_80) 
				!= vht_cent_ch_freq(Bw8080ChAvoid, VHT_BW_80)
				))
				{				
			...
			MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s]:No immediately available Channel. Select Ch %d\n",
					__FUNCTION__, ch));
