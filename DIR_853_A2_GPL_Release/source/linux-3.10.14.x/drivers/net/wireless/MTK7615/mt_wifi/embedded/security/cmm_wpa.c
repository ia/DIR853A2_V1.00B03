        ...
        if (IS_AKM_PSK(pSecConfig->AKMMap) || IS_AKM_1X(pSecConfig->AKMMap))
        {
			...
			if (Elem == NULL) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
					 ("%s:Alloc mlme queue elem Fail!!!\n", __func__));
				break;
			}

			pInfo = (MLME_DEAUTH_REQ_STRUCT *)Elem->Msg;
			os_move_mem(pInfo->Addr, pEntry->Addr, MAC_ADDR_LEN);
			pInfo->Reason = REASON_4_WAY_TIMEOUT;
			Elem->Wcid = pEntry->wcid;
			
			MlmeEnqueueWithWcid(pAd, AP_AUTH_STATE_MACHINE, APMT2_MLME_DEAUTH_REQ,
						sizeof(MLME_DEAUTH_REQ_STRUCT), Elem, 0);
			RTMP_MLME_HANDLER(pAd);
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("%s:Send Deauth to %02x:%02x:%02x:%02x:%02x:%02x.(Reason=%d)\n",
				  __func__, PRINT_MAC(pInfo->Addr), pInfo->Reason));
