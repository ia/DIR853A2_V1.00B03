...
//modify start by wf
	//if (multi_inf_get_idx(pAd) == 0)//2.4G
	{
		pPath = SINGLE_SKU_TABLE_FILE_NAME_25G;
	}
	//else if (multi_inf_get_idx(pAd) == 1)//5.8G
	//{
	//	pPath = SINGLE_SKU_TABLE_FILE_NAME_5G;
	//}	
	//else
	//{
	//	pPath = SINGLE_SKU_TABLE_FILE_NAME;
	//}
	printk(" wf single load file %s\n", pPath);
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: SINGLE SKU TABLE FILE %s!!!\n", __FUNCTION__, pPath));

	/* open card information file*/
	//srcf = os_file_open(SINGLE_SKU_TABLE_FILE_NAME, O_RDONLY, 0);
	srcf = os_file_open(pPath, O_RDONLY, 0);
	if (srcf.Status)
	{
...
INT	MtBfBackOffLoadTable(RTMP_ADAPTER *pAd)
{
	...
	//	if (multi_inf_get_idx(pAd) == 0)//2.4G
		{
			pPath = BF_SKU_TABLE_FILE_NAME_25G;
		}
	//	else if (multi_inf_get_idx(pAd) == 1)//5G
       //         {
       //                 pPath = BF_SKU_TABLE_FILE_NAME_5G;
       //         }
	//	else 
	//	{
	//		pPath = BF_SKU_TABLE_FILE_NAME;
	//	}
printk(" wf single load file %s\n", pPath);	
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: BF SKU TABLE FILE %s!!!\n", __FUNCTION__, pPath));
