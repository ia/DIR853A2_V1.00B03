...
ssize_t proc_wsc_status_set(struct file *file, const char __user *buffer, size_t count, loff_t *ppos);

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
ssize_t proc_wsc_status_get(struct file *file, char __user *buffer, size_t count, loff_t *ppos);
static const struct file_operations proc_wsc_status_fops = {
        .read = proc_wsc_status_get,
        .write = proc_wsc_status_set,
};
#else
ssize_t proc_wsc_status_get(char *page, char **start, off_t off, int count, int *eof, void *data_unused);
#endif
/* TW 20161103 */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
               entry_wsc_status = proc_create("wsc_status", 0x0644, proc_dir_wsc, &proc_wsc_status_fops);
#else
...
/*2017-05-27 linsd add for associated status*/
struct proc_dir_entry *proc_apclient;
extern int wlconfig_assoc_flag;
static struct proc_dir_entry *entry_apcli_status;

ssize_t proc_apcli_status_get(struct file *file, char __user *buffer,
                                               size_t count, loff_t *ppos)
{
    int iLen = 0;
...
ssize_t proc_apcli_status_set(struct file *file, const char __user *buffer,
                                               size_t count, loff_t *ppos)
{
       char *buf = kmalloc(count, GFP_KERNEL);
       ...
       if (buf) {
           ...
           wlconfig_assoc_flag = val;
       }
       return count;
}

static const struct file_operations proc_apcli_status_fops = {
        .read = proc_apcli_status_get,
        .write = proc_apcli_status_set,
};


int apclient_proc_init(void)
{
	if(proc_apclient == NULL)
		proc_apclient = proc_mkdir("apclient", NULL);

	if(proc_apclient)
	{
		entry_apcli_status = proc_create("status", 0x0644, proc_apclient, &proc_apcli_status_fops);
	}

	return 0;
}

int apclient_proc_exit(void)
{
	if (proc_apclient){
		remove_proc_entry("status", proc_apclient);
	}

	remove_proc_entry("apclient", NULL);
